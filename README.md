...
- **创建文档**：创建新 Issue
- **编辑文档**：编辑 Issue 内容
- **分类文档**：通过 Issue 标题或 GitHub 标签
- **删除文档**：关闭或删除 Issue

### 文档标题和标签规则

#### 文档标题
- 如果 Issue 内容中包含 Markdown 一级标题（`# 标题`），将使用该标题作为文档标题
- 否则使用 Issue 的标题作为文档标题

**示例**：
```markdown
Issue 标题：前端 开发 React教程
...
#### 标签（分类）
标签来自两个来源，会自动合并：
1. **Issue 标题中的词语**（以空格分隔）
2. **GitHub 手动添加的标签**

**示例**：
- Issue 标题：`前端 React 教程`
- GitHub 标签：`tutorial`, `javascript`
- 最终标签：`前端`, `React`, `教程`, `tutorial`, `javascript`
...
- **标签索引页** (`docs/tags.md`) - 列出所有标签及文章数量
- **标签详情页** (`docs/tags/{标签名}.md`) - 列出该标签下的所有文章
- 支持标签间跳转和文章链接
- 点击标签可查看相关文章列表
